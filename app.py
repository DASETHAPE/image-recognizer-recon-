import streamlit as st
import numpy as np
from PIL import Image
from tensorflow.keras.applications.mobilenet_v2 import MobileNetV2, preprocess_input, decode_predictions
from tensorflow.keras.preprocessing.image import img_to_array

st.set_page_config(page_title="Universal Object Recognizer", page_icon="🌍")
st.title("Universal Object Recognizer 🌍")
st.write("Upload an image of almost anything (dog, car, coffee mug, etc.) and the AI will guess what it is!")

# 1. Load the pre-trained MobileNetV2 model (it downloads automatically!)
@st.cache_resource
def load_model():
    return MobileNetV2(weights='imagenet')

model = load_model()

uploaded_file = st.file_uploader("Choose an image...", type=["jpg", "png", "jpeg"])

if uploaded_file is not None:
    # Display the uploaded image
    image = Image.open(uploaded_file)
    st.image(image, caption='Uploaded Image', use_column_width=True)
    
    # 2. Preprocess the image to 224x224 RGB
    image = image.resize((224, 224))
    
    # Ensure the image has 3 color channels (RGB)
    if image.mode != 'RGB':
        image = image.convert('RGB')
        
    img_array = img_to_array(image)
    img_array = np.expand_dims(img_array, axis=0)
    
    # Apply the specific mathematical preprocessing MobileNetV2 expects
    img_array = preprocess_input(img_array)
    
    if st.button('Predict Object'):
        # Make the prediction
        prediction = model.predict(img_array)
        
        # 3. Decode the prediction into English words (Get the top 3 guesses)
        results = decode_predictions(prediction, top=3)[0]
        
        st.success(f"**Top Prediction: {results[0][1].replace('_', ' ').title()}**")
        st.info(f"Confidence: {results[0][2] * 100:.2f}%")
        
        st.write("Other possibilities:")
        for i in range(1, 3):
            st.write(f"- {results[i][1].replace('_', ' ').title()}: {results[i][2] * 100:.2f}%")
